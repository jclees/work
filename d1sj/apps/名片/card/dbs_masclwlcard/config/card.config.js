var n = {
    green: {
        mainColor: "#1AAD19",
        borderColor: "#1AAD19",
        bgimg: "https://img.jikni.com/w1.png",
        footernav: {
            product: {
                footer_nav: [ "名片", "产品", "动态", "官网" ],
                nav_footer: [ {
                    name: "名片",
                    icon: "../../images/card/01_2x_n.png",
                    selectIcon: "../../images/card/01_2x.png"
                }, {
                    name: "产品",
                    icon: "../../images/card/02_x_n.png",
                    selectIcon: "../../images/card/02_x.png"
                }, {
                    name: "动态",
                    icon: "../../images/card/03_x_n.png",
                    selectIcon: "../../images/card/03_x.png"
                }, {
                    name: "官网",
                    icon: "../../images/card/04_x_n.png",
                    selectIcon: "../../images/card/04_x.png"
                } ]
            },
            shopping: {
                footer_nav: [ "名片", "商城", "动态", "官网" ],
                nav_footer: [ {
                    name: "名片",
                    icon: "../../images/common_tab_btn_card_n.png",
                    selectIcon: "../../images/common_tab_btn_card_s.png"
                }, {
                    name: "商城",
                    icon: "../../images/common_tab_btn_store_n.png",
                    selectIcon: "../../images/common_tab_btn_store_n_copy.png"
                }, {
                    name: "动态",
                    icon: "../../images/common_tab_btn_discover_n.png",
                    selectIcon: "../../images/common_tab_btn_discover_s.png"
                }, {
                    name: "官网",
                    icon: "../../images/common_tab_btn_officialwebsite_n.png",
                    selectIcon: "../../images/common_tab_btn_officialwebsite_s.png"
                } ]
            }
        }
    }
};

module.exports = n;