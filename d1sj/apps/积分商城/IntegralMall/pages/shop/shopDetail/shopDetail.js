...
Page({
  ...
  getDetail() {
    common.get('/product', {
      ...
      this.setData({
        detailList: list,
        'detailList.detail': list.detail.replace(/\<img/g, '<img style="width:100%;height:auto;display:block"'),
        'detailList.num': 1
      })
  ...
  like() {
    ...
    if (detailList.like == 1) {
      url = '/collection/cancel_collect'
      param = {
        member_id: this.data.member_id,
        id: this.data.detailList.id
      }
    } else {
      url = '/collection/to_collect'
      param = {
        member_id: this.data.member_id,
        ids: [this.data.detailList.id]
      }
    }
    common.post(url, param).then(res => {
      console.log("收藏")
      console.log(res)
      if (res.data.code == 200) {
        if (detailList.like == 1) {
          _this.data.detailList.like = 0
        }else{
          _this.data.detailList.like = 1
        }
      }
      _this.setData({
